...
DESCRIPTION:			HW 1
NOTE:					50% Each
						Sean: polygon filled, line segment, circle
						David: Bitmap, store pixels, text, pixel map
...
void background()
{
	...
	for (int i = 75; i <95; i++)
	{
		...
		{
			myPixelmap[i][j][0] = 0.6;
			myPixelmap[i][j][1] = 0;
			myPixelmap[i][j][2] = 0.85;
		}
	}
	for (int i = 75; i <95; i++)
	{
		...
		{
			myPixelmap[i][j][0] = 0.7;
			myPixelmap[i][j][1] = 0.8;
			myPixelmap[i][j][2] = 0.2;
		}
	}
	for (int i = 82; i <87; i++)
	{
...
void myDisplayCallback()
{
	...
	ofstream myfile;
	myfile.open("C:\\TEMP\\savedImg.txt");
	for (int x =0; x<512; x++)
	{
		for (int y=0; y<512; y++)
		{
...
void main(int argc, char ** argv)
{
	...
	glutCreateWindow("Let it Snow");	// create a titled window
